...
public class DrawGraph extends JPanel {
	...
	public void paintComponent(Graphics graphics) {
        ...
        for(int i = 1; i < theDataset.size(); i++){
        	...
        	if(theDataset.get(i-1).getMed() != 0 && theDataset.get(i).getMed() != 0){
        		//draw median line
        		g2.draw(new Line2D.Double(((double) theDataset.get(i-1).getTimestamp() - xMin) * width / distX,
        			height - ((theDataset.get(i-1).getMed() - yMin)*height/distY),
        			((double) theDataset.get(i).getTimestamp() - xMin) * width / distX,
        			height- ((theDataset.get(i).getMed() - yMin)*height/distY)));
        	}
        }
	}
}
