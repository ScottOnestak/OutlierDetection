...
public class MovingMedianAverage {
	...
	public static void main(String[] args) {
		...
		//set equal to 67 for real data files, 100 for everything else
		for(int i = 1; i <= 20; i++){
	...
	public static void movingMedianAverage(int size){
		for(int i = size + 1; i < theDataset.size() - 1; i++){
			double theMedian = median(i-size,i-1);
	...
	public static void calcRes(int size){
		...
		for(int i = size; i < theDataset.size(); i++){
			double res = theDataset.get(i).getValue() - theDataset.get(i).getMed();
